...
export function OrderListPage(props: OrderListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getOrders({
        relations: ['user_order_customer_idTouser', 'user_order_sales_associate_idTouser', 'order_item.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_order_sales_associate_idTouser',
          header: 'User Order Sales Associate Id Touser',
          accessorKey: 'user_order_sales_associate_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_order_sales_associate_idTouser?.id}`}
            >
              {record.user_order_sales_associate_idTouser?.email}
            </Link>
          ),
        }
      : null,
